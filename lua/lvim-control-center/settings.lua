-- lvim-control-center.settings: the runtime-configurable UI geometry — the shared `config.ui.size`, per layout a size
...
--
---@module "lvim-control-center.settings"
...
local function root_tbl(root)
    root = root or "size"
    local ui = require("lvim-ui.config")
    ui[root] = ui[root] or {}
    return ui[root]
end
