-- lvim-control-center.config_ui: the :LvimUtils config settings panel.
--
...
--
---@module "lvim-control-center.config_ui"

local settings = require("lvim-control-center.settings")
...
function M.open()
    ...
    if not ok then
        vim.notify("lvim-control-center: the config panel needs the lvim-ui module", vim.log.levels.WARN)
        return
    end
...
--- Register the `:LvimUtils` config-panel command. Idempotent (called from lvim-control-center.setup).
function M.setup()
