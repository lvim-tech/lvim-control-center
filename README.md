...
| `options` | `any[]` or `fun(): any[]` | (For `type="select"`) The list of possible values. A **function** is evaluated when the panel opens — use it when the list comes from another plugin that may not be loaded while your group module is required. |
